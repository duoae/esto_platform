...
class UpdateLabRequest extends FormRequest
{
    ...
    {
        $lab = $this->route('lab') ?? $this->route('laboratoire');
        $labId = is_object($lab) ? $lab->id : $lab;

        return [
            'acronym'        => 'nullable|string|max:50|unique:laboratoires,acronyme,' . $labId,
            'acronyme'       => 'sometimes|required|string|max:50|unique:laboratoires,acronyme,' . $labId,
