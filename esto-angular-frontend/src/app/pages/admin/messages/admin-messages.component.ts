        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
          ...
          <button *ngFor="let conv of filteredConversations" 
            ...
            <div class="relative shrink-0">
              <img *ngIf="conv.photo_profil" [src]="getDocUrl(conv.photo_profil)" class="w-9 h-9 rounded-full object-cover shrink-0 border"
                   [ngClass]="selectedConversation?.id === conv.id ? 'border-white shadow-sm' : 'border-slate-200'">
        ...
        <ng-container *ngIf="selectedConversation">
          ...
          <div class="p-3 border-b border-slate-100 flex justify-between items-center bg-white/90 backdrop-blur-sm z-10 sticky top-0">
            <div class="flex items-center gap-3">
              ...
              <img *ngIf="selectedConversation.photo_profil" [src]="getDocUrl(selectedConversation.photo_profil)" class="w-9 h-9 rounded-full object-cover shrink-0 border border-teal-100 shadow-sm">
          ...
          <div class="flex-1 overflow-y-auto p-4 bg-slate-50/50 flex flex-col space-y-4 custom-scrollbar" #chatContainer>
            ...
            <div *ngFor="let msg of messages" class="flex w-full" [ngClass]="msg.is_mine ? 'justify-end' : 'justify-start'">
              ...
              <div class="flex flex-col max-w-[85%] md:max-w-[75%]">
                <div class="flex items-end gap-2" [ngClass]="msg.is_mine ? 'flex-row-reverse' : 'flex-row'">
                  
                  <!-- Message Sender Avatar (if not mine) -->
                  <img *ngIf="!msg.is_mine && msg.sender_photo_profil" [src]="getDocUrl(msg.sender_photo_profil)" class="w-7 h-7 rounded-full object-cover shrink-0 mb-1 border border-slate-200 shadow-sm">
                  <div *ngIf="!msg.is_mine && !msg.sender_photo_profil" class="w-7 h-7 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center font-bold text-[10px] shrink-0 mb-1 border border-slate-300 shadow-sm">
                  ...
                  <div class="absolute top-1/2 -translate-y-1/2 flex items-center gap-1 z-10 h-full"
    ...
    <div *ngIf="showNewMessageModal" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h2 class="text-xl font-bold text-gray-900">Nouveau Message (Admin)</h2>
        ...
        <form (ngSubmit)="sendNewMessage()" class="flex flex-col flex-1 overflow-hidden p-6 space-y-5 overflow-y-auto">
          ...
          <div class="mb-5">
            ...
            <div class="flex w-full gap-2">
              <button type="button" (click)="setMode('all_candidats')"
                      [ngClass]="newMsgForm.mode === 'all_candidats' ? 'bg-teal-500 text-white border-teal-500 shadow-sm hover:bg-teal-600' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300'"
                      class="flex-1 px-1 py-2 text-[10px] sm:text-xs border rounded-xl font-medium transition-all text-center break-words leading-tight">
                Tous les Candidats
              </button>
              <button type="button" (click)="setMode('all_doctorants')"
                      [ngClass]="newMsgForm.mode === 'all_doctorants' ? 'bg-teal-500 text-white border-teal-500 shadow-sm hover:bg-teal-600' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300'"
                      class="flex-1 px-1 py-2 text-[10px] sm:text-xs border rounded-xl font-medium transition-all text-center break-words leading-tight">
                Tous les Doctorants
              </button>
              <button type="button" (click)="setMode('all_professeurs')"
                      [ngClass]="newMsgForm.mode === 'all_professeurs' ? 'bg-teal-500 text-white border-teal-500 shadow-sm hover:bg-teal-600' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300'"
                      class="flex-1 px-1 py-2 text-[10px] sm:text-xs border rounded-xl font-medium transition-all text-center break-words leading-tight">
                Tous les Profs
              </button>
              <button type="button" (click)="setMode('all_directeurs')"
                      [ngClass]="newMsgForm.mode === 'all_directeurs' ? 'bg-teal-500 text-white border-teal-500 shadow-sm hover:bg-teal-600' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300'"
                      class="flex-1 px-1 py-2 text-[10px] sm:text-xs border rounded-xl font-medium transition-all text-center break-words leading-tight">
                Tous les Directeurs
              </button>
              <button type="button" (click)="setMode('all')"
                      [ngClass]="newMsgForm.mode === 'all' ? 'bg-teal-500 text-white border-teal-500 shadow-sm hover:bg-teal-600' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300'"
                      class="flex-1 px-1 py-2 text-[10px] sm:text-xs border rounded-xl font-medium transition-all text-center break-words leading-tight">
                Tout le monde
              </button>
            </div>
          ...
          <div class="mb-5 relative z-50">
            ...
            <div *ngIf="showUserDropdown" class="absolute z-50 mt-1 w-full bg-white rounded-xl shadow-lg border border-gray-100 max-h-60 overflow-y-auto custom-scrollbar">
              ...
              <button *ngFor="let u of flatFilteredUsers" type="button" (mousedown)="selectUser(u)" class="w-full text-left px-4 py-3 hover:bg-teal-50 border-b border-gray-50 flex items-center gap-3 transition-colors">
                <img *ngIf="u.photo_profil" [src]="getDocUrl(u.photo_profil)" class="w-8 h-8 rounded-full object-cover shrink-0 border border-slate-200">
                <div *ngIf="!u.photo_profil" class="w-8 h-8 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center font-bold text-xs shrink-0">
                  {{ (u.nom || 'U').substring(0,1).toUpperCase() }}
                </div>
                <div class="flex flex-col flex-1">
                  <span class="font-bold text-sm text-gray-900 leading-tight">{{ u.nom }} {{ u.prenom }}</span>
                  <span class="text-xs text-gray-500">{{ u.email ? u.email : "Pas d'email" }}</span>
                </div>
                <span class="text-[9px] px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full font-bold uppercase tracking-wider shrink-0">{{ u.role }}</span>
              </button>
            </div>
            
            <div *ngIf="newMsgForm.mode === 'individual' && newMsgForm.receiver_id" class="mt-2 inline-flex items-center gap-2 px-3 py-1.5 bg-teal-50 text-teal-600 rounded-lg text-sm font-medium border border-teal-100">
          ...
          <div class="pt-2">
             ...
             <div *ngIf="newMsgForm.file" class="mt-3 p-3 bg-teal-50 rounded-xl border border-teal-100 flex items-center justify-between">
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-sm text-teal-700 font-bold">{{ newMsgForm.file.name }}</span>
                </div>
          ...
          <div class="pt-4 flex justify-end gap-3 mt-auto">
            ...
            <button type="submit" 
                    [disabled]="isSending"
                    class="px-6 py-2.5 rounded-xl text-white bg-teal-500 hover:bg-teal-600 shadow-md font-bold transition-all flex items-center gap-2"
...
export class AdminMessagesComponent implements OnInit {
  ...
  newMsgForm: {
    ...
    mode: 'all_candidats' | 'all_doctorants' | 'all_professeurs' | 'all_directeurs' | 'all' | 'individual';
    titre: string;
    contenu: string;
    file: File | null;
  } = {
  ...
  setMode(mode: 'all_candidats' | 'all_doctorants' | 'all_professeurs' | 'all_directeurs' | 'all' | 'individual') {
  ...
  closeNewMessageModal() {
    ...
    this.newMsgForm = { mode: 'all_candidats', titre: '', contenu: '', receiver_id: null, file: null };
  }
  ...
  sendNewMessage() {
    ...
    if (this.newMsgForm.mode === 'individual') {
      ...
      this.http.post<any>(`${this.apiUrl}/messages/broadcast`, formData, {
        headers: { Authorization: `Bearer ${localStorage.getItem('esto_token')}` }
      }).subscribe({
        next: (res) => {
          this.isSending = false;
  ...
  getFileName(path: string): string {
    ...
    const nameParts = fileWithTimestamp.split('_');
