        ...
        <form (ngSubmit)="sendNewMessage()" class="flex flex-col flex-1 overflow-hidden p-6 space-y-5 overflow-y-auto">
          ...
          <div class="mb-5">
            ...
            <div class="flex flex-wrap gap-2">
              <button type="button" (click)="setMode('lab_professeurs')"
                      [ngClass]="newMsgForm.mode === 'lab_professeurs' ? 'bg-esto-primary text-white border-esto-primary shadow-sm hover:bg-esto-primary/90' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300'"
              ...
              <button type="button" (click)="setMode('lab_doctorants')"
                      [ngClass]="newMsgForm.mode === 'lab_doctorants' ? 'bg-esto-primary text-white border-esto-primary shadow-sm hover:bg-esto-primary/90' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300'"
                      class="px-4 py-2 text-sm border rounded-xl font-medium transition-all">
                Tous les Doctorants du Labo
              </button>
              <button type="button" (click)="setMode('lab_all')"
                      [ngClass]="newMsgForm.mode === 'lab_all' ? 'bg-esto-primary text-white border-esto-primary shadow-sm hover:bg-esto-primary/90' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300'"
                      class="px-4 py-2 text-sm border rounded-xl font-medium transition-all">
                Tout le Labo
              </button>
            </div>
          ...
          <div class="mb-5 relative z-50">
            ...
            <input type="text" [(ngModel)]="searchUserQuery" name="searchUserQuery" 
                   ...
                   autocomplete="off"
                   class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-esto-primary focus:ring-esto-primary sm:text-sm py-2.5 px-4 bg-gray-50/50"
            ...
            <div *ngIf="showUserDropdown" class="absolute z-50 mt-1 w-full bg-white rounded-xl shadow-lg border border-gray-100 max-h-60 overflow-y-auto custom-scrollbar">
              ...
              <button *ngFor="let u of flatFilteredUsers" type="button" (mousedown)="selectUser(u)" class="w-full text-left px-4 py-3 hover:bg-esto-primary/5 border-b border-gray-50 flex items-center justify-between transition-colors">
            ...
            <div *ngIf="newMsgForm.mode === 'individual' && newMsgForm.receiver_id" class="mt-2 inline-flex items-center gap-2 px-3 py-1.5 bg-esto-primary/10 text-esto-primary rounded-lg text-sm font-medium border border-esto-primary/20">
          ...
          <div class="flex-1 flex flex-col min-h-[150px]">
            ...
            <textarea [(ngModel)]="newMsgForm.contenu" name="contenu" required rows="5"
                      class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-esto-primary focus:ring-esto-primary sm:text-sm py-3 px-4 bg-gray-50/50 resize-none flex-1 custom-scrollbar"
          ...
          <div class="pt-2">
             ...
             <div *ngIf="newMsgForm.file" class="mt-3 p-3 bg-esto-primary/5 rounded-xl border border-esto-primary/20 flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <svg class="w-5 h-5 text-esto-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                  <span class="text-sm text-esto-primary font-bold">{{ newMsgForm.file.name }}</span>
                </div>
          ...
          <div class="pt-4 flex justify-end gap-3 mt-auto">
            ...
            <button type="submit" 
                    [disabled]="isSending"
                    class="px-6 py-2.5 rounded-xl text-white bg-esto-primary hover:bg-esto-primary/90 shadow-md font-bold transition-all flex items-center gap-2"
