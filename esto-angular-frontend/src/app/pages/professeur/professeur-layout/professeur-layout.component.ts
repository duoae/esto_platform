...
export class ProfesseurLayoutComponent implements OnInit, OnDestroy {
  user: any = null;
  ...
  ngOnInit() {
    ...
    if (userStr) {
      ...
      if (this.user.role !== 'professeur' && this.user.role !== 'directeur') {
        if (this.user.role === 'candidat' || this.user.role === 'doctorant') this.router.navigate(['/candidat/mes-candidatures']);
  ...
  checkUnreadMessages(): void {
    ...
    if (!token) return;
    this.http.get<any>(`${this.apiUrl}/messages/unread?role_context=professeur`, {
